---
layout: single
author_profile: false
classes: wide
---

<div style="max-width: 800px; margin: 0 auto; padding: 2rem 0;">
  <h1 style="font-size: 2.5rem; margin-bottom: 0.5rem;">Dheeraj Neelam</h1>

  <p style="font-size: 1.25rem; color: #666; margin-bottom: 2rem;">
    Software engineer passionate about building great products and sharing knowledge
  </p>

  <div style="margin-bottom: 3rem;">
    <p style="line-height: 1.7; color: #444;">
      I build iOS apps and work on distributed systems. I share what I learn through blog posts, quick tips, and open source contributions.
    </p>
  </div>

  <div style="display: flex; gap: 2rem; flex-wrap: wrap;">
    <a href="https://github.com/dheerajn" target="_blank" rel="noopener" style="text-decoration: none; color: #333; font-size: 1.1rem; display: flex; align-items: center; gap: 0.5rem;">
      <i class="fab fa-github" style="font-size: 1.5rem;"></i> GitHub
    </a>
    <a href="https://www.linkedin.com/in/dheerun/" target="_blank" rel="noopener" style="text-decoration: none; color: #333; font-size: 1.1rem; display: flex; align-items: center; gap: 0.5rem;">
      <i class="fab fa-linkedin" style="font-size: 1.5rem;"></i> LinkedIn
    </a>
    <a href="https://x.com/DheeruN1210" target="_blank" rel="noopener" style="text-decoration: none; color: #333; font-size: 1.1rem; display: flex; align-items: center; gap: 0.5rem;">
      <i class="fab fa-twitter" style="font-size: 1.5rem;"></i> X
    </a>
  </div>
</div>
